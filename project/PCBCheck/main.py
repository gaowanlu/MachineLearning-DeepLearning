...
def main():
    ...
    # 数据准备
    yes_data = load_img_set(
        YES_SIZE, "C:\\Users\\gaowanlu\\Desktop\\MyProject\\A10\\fun\\yes\\yes ({}).jpg")
    no_data = load_img_set(
        NO_SIZE, "C:\\Users\\gaowanlu\\Desktop\\MyProject\\A10\\fun\\no\\no ({}).jpg")
    origin_data = []
    ...
    x_train, x_test, y_train, y_test = train_test_split(
        X_reduced, y_train_transed, test_size=0.4)
    ...
    for index in range(len(origin_data)):
        ...
        print("Result : ", pred)
        img = cvPy.get_putText(origin_data[index], '{}'.format(
            pred[0]), (200, 200), 3, (255, 0, 0), 2)
        # 图片聚类 k=2
        gray_img = cvPy.get_Gray(img)
        all = cv2.imread(
            "C:\\Users\\gaowanlu\\Desktop\\MyProject\\A10\\fun\\all.jpg", cv2.IMREAD_COLOR)
        ...
        thresh1 = cv2.inRange(hsv, np.array(
            [30, 37, 36]), np.array([77, 255, 255]))
...
def all(index):
    ...
    all = cv2.imread("C:\\Users\\gaowanlu\\Desktop\\MyProject\\A10\\all\\all ({}).jpg".format(
        index), cv2.IMREAD_COLOR)
    # 通道分离
